...
import kotlinx.android.synthetic.main.activity_add_new_reservation.mail_txt
        ...
        if(title_ar.isNotEmpty()&&title_en.isNotEmpty()&&price.isNotEmpty()){
            ...
            apiClient.getApiService(this).addPharmOffer(title_ar,title_en,price,img)
                .enqueue(object : Callback<BaseResponce<PharmAddOfferModel>> {
                    override fun onFailure(call: Call<BaseResponce<PharmAddOfferModel>>, t: Throwable) {
                        alertNetwork(true)
                    }
                    ...
                    override fun onResponse(
                        ...
                        if (response!!.isSuccessful) {
                            if (response.body()!!.success) {
                                Toast.makeText(this@AddNewOfferActivity, "SUCCESS", Toast.LENGTH_SHORT).show()
                                intent= Intent(this@AddNewOfferActivity, MainActivity::class.java)
                                startActivity(intent)
                            } else {
                                Toast.makeText(this@AddNewOfferActivity, "faild", Toast.LENGTH_SHORT).show()
                            }
